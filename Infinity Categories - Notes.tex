...
\end{tikzcd}$$
that $\varepsilon_{ijk}\circ\varepsilon_{ikl}=\varepsilon_{jkl}\circ\varepsilon_{ikl}$. More explicitly, we have natural transformations 
\begin{center}
\begin{tabular}{c c}
  $\varepsilon_{ijk}:F_{ik}\to F_{jk}\circ F_{ij}$ & $\varepsilon_{ikl}:F_{il}\to F_{kl}\circ F_{ik}$ \\
  $\varepsilon_{jkl}:F_{jl}\to F_{kl}\circ F_{jk}$ & $\varepsilon_{ijl}:F_{il}\to F_{jl}\circ F_{ij}$
\end{tabular}
\end{center}
where the ``cocycle condition'' asserts that the compositions of functors as they arise from different composite functors agree where composing the natural transformations to the ``longest chain'' in two different ways
$$\varepsilon_{jkl}\circ\varepsilon_{ikl}:F_{il}\to F_{kl}\circ F_{jk}\circ F_{ij}$$
$$\varepsilon_{jkl}\circ\varepsilon_{ikl}:F_{il}\to F_{kl}\circ F_{jk}\circ F_{ij}$$
agree, as one would naturally expect. 
\begin{remark}
...
\begin{definition}
  Let $\Csf,\Dsf$ be categories. The functor category $\Mor_{\Cat}(\Csf,\Dsf)$ has objects functors $\Csf\to\Dsf$ and morphisms natural transformations between such functors. 
\end{definition}
In the case of simplicial sets, this admits an even nicer description. Let $X,Y$ be simplicial sets. $\Mor_{\SSets}(X,Y)=Y^{X}$ is itself a simplicial set such that maps $Z\to Y^{X}$ can be identified with maps $X\times Z\to Y$. More generally, one can show that for $\Csf,\Dsf$ categories $\Mor_{\Cat}(N\Csf, N\Dsf)=N\Dsf^{N\Csf}$ is isomorphic to the nerve of the functor category $N \Mor_{\Cat}(\Csf,\Dsf)$. In the case of quasicategories, a theorem of Joyal states that we have the following. 
\begin{theorem}[Joyal]
  If $X$ is a quasicategory and $A$ a simplical set, the functor category $\Mor_{\Cat}(A,X)=X^{A}$ is a quasicategory. 
\end{theorem}
\newpage
\printbibliography
\end{document}